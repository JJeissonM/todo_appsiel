...
- Excepción de cierre: en **Inventario Físico**, si el cajero ya no tiene un turno abierto, el formulario asigna automáticamente su último turno `CERRADO`. Esto permite registrar el conteo de entrega con la apertura, cierre y fecha operativa reales del turno que acaba de finalizar. El cajero no puede seleccionar otro turno y esta excepción no se aplica a ventas, tesorería ni a otras transacciones.
- Los **arqueos de caja** se realizan sobre turnos que ya tienen cierre. Al abrir el formulario se selecciona el último turno cerrado correspondiente a la caja y PDV, aunque su fecha operativa sea anterior al día calendario actual (por ejemplo, un turno nocturno). La fecha y las horas del arqueo se toman del turno; al cambiar manualmente la fecha se muestran únicamente los turnos cerrados de ese día.
