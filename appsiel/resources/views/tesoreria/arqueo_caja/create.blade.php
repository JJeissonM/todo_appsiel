...
@section('campos_adicionales')
	...
	<div class="container-fluid">
					...
					<tbody>
						@foreach($array_billetes as $key => $value)
							<tr>
								...
								<td class="col-md-4">
									<input type="number" min="0" class="form-control cantidad_b" id="billete_{{$value}}" autocomplete="off" name="billetes[]">									
								</td>
								<td class="col-md-6">
									<div class="lbl_total_b"> $0 </div>
									<input type="hidden" class="total_b" value="0">
								</td>
							</tr>
						@endforeach
					</tbody>
					<tfoot>
						<tr class="success">
							...
							<td> 
								...
								<input type="hidden" id="total_billetes" value="0">
							</td>
						</tr>
					</tfoot>
				</table>
				...
				<table class="table table-striped table-bordered table-hover table-condensed">
					...
					<tbody>
						@foreach($array_monedas as $key => $value)
							@if($value == '')
								<tr> <td>&nbsp;</td> <td> <input type="number" min="0" class="form-control" disabled="disabled" > </td> <td>&nbsp;</td></tr>
							@else
								<tr>
									<td class="col-md-2">
										${{ number_format($value,'0',',','.') }}
									...
									<td class="col-md-4">
										<input type="number" min="0" class="form-control cantidad_m" id="billete_{{$value}}" autocomplete="off" name="monedas[]">									
									</td>
									<td class="col-md-6">
										<div class="lbl_total_m"> $0 </div>
										<input type="hidden" class="total_m" value="0">
									</td>
								</tr>
							@endif
						@endforeach
					...
					<tfoot>
						<tr class="success">
							...
							<td> 
								...
								<input type="hidden" id="total_monedas" value="0">
							</td>
						</tr>
					</tfoot>
				</table>
			...
			<tbody>
				<tr>
					<td class="col-md-6">
						<b>Saldo Total</b>
						<br>
						<input type="text" min="0" class="form-control otros_saldos" id="otros_saldos" autocomplete="off" name="otros_saldos" placeholder="$">									
					</td>
					<td class="col-md-6">
						<b>Observaciones</b>
						<br>
		...
		<div class="well">
			<h1>Total efectivo físico: <div id="lbl_total_efectivo" style="display: inline;"> $0 </div></h1>
			<input type="hidden" id="total_efectivo" value="0">
		</div>
		...
		<div class="row">
			<div class="col-md-6">
				...
				<div class="well">
					<h3 style="text-align: center;"> Mov. de entradas de efectivo <small> <a class="btn btn-xs btn-primary" id="btn_get_mov_entrada"> Obtener </a> </small> <small> <a class="btn btn-xs btn-info" id="btn_reset_mov_entrada"> Reset </a> </small> </h3>
					<div id="div_mov_entrada">
						
					</div>
					<input type="hidden" id="total_mov_entradas" value="0">
				</div>
			...
			<div class="col-md-6">
				...
				<div class="well">
					<h3 style="text-align: center;"> Mov. de salidas de efectivo <small> <a class="btn btn-xs btn-primary" id="btn_get_mov_salida"> Obtener </a> </small> <small> <a class="btn btn-xs btn-info" id="btn_reset_mov_salida"> Reset </a> </small> </h3>
					<div id="div_mov_salida">
						
					</div>
					<input type="hidden" id="total_mov_salidas" value="0">
				</div>
		...
		<div class="well">
			<h1>Saldo en el sistema: <div id="lbl_total_sistema" style="display: inline;"> $0 </div></h1>
			<input type="hidden" id="total_sistema" value="0">
		</div>

		<div class="well">
			<h1>Diferencia: <div id="lbl_total_saldo" style="display: inline;"> $0 </div></h1>
			<input type="hidden" id="total_saldo" value="0">
		</div>
	...
	<script type="text/javascript">
		...
		$(document).ready(function(){
			...
			$('#btn_get_mov_salida').on('click',function(event){
				...
				$.get( url, { movimiento: 'salida', fecha_desde: $('#fecha').val(), fecha_hasta: $('#fecha').val(), teso_caja_id: $('#teso_caja_id').val() } )
					.done(function( respuesta ) {
						$('#div_cargando').hide();
