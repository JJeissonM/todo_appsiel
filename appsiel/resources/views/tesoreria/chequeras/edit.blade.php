...
@section('content')
    <div class="container-fluid">
        <h4>Editar chequera</h4>
        <p><b>Cuenta bancaria:</b> {{ $cuenta->get_value_to_show() }}</p>
        <hr>

        @include('layouts.mensajes')

        {{ Form::model($chequera, ['url' => 'teso_cuentas_bancarias/'.$cuenta->id.'/chequeras/'.$chequera->id.'?id='.Input::get('id').'&id_modelo='.Input::get('id_modelo').'&id_transaccion='.Input::get('id_transaccion'), 'method' => 'PUT']) }}
            <div class="row">
                <div class="col-md-6">
