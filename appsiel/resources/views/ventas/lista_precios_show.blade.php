...
@section('content')
	...
	<?php 
		...
		*/
		...
		$contenido_modal = Form::open(['url'=>'vtas_agregar_precio_lista','id'=>'form_create','files' => true]).Form::bsSelect('inv_producto_id', null, 'Producto', App\Inventarios\InvProducto::opciones_campo_select(), ['required'=>'required']).'<br><br><br>'.Form::bsFecha('fecha_activacion',null,'Fecha activación',[],['required'=>'required']).'<br><br>'.Form::bsText('precio',null, 'Precio', ['required'=>'required'] ).Form::hidden('lista_precios_id', $registro->id).Form::hidden('url_id',Input::get('id')).Form::hidden('url_id_modelo', Input::get('id_modelo')).Form::hidden('url_id_transaccion', Input::get('id_transaccion')).'<br><br><br>'.Form::close();
