            ...
            <tr>
                ...
                <td style="width:20%;">
                    ...
                    @if( (int)config('ventas_pos.imprimir_pedidos_en_cocina') )
                        <a class="btn btn-default btn-xs btn-detail btn_imprimir_en_cocina" href="{{ url( 'vtas_pedidos_imprimir/' . $pedido->id . '?id=20&id_modelo=175&id_transaccion=42&formato_impresion_id=pos' ) }}" title="Imprimir en cocina" id="btn_print" data-lbl_consecutivo_doc_encabezado="{{ $pedido->consecutivo }}" data-lbl_fecha="{{ $pedido->fecha }}" data-lbl_cliente_descripcion="{{ $pedido->tercero->descripcion }}" data-lbl_descripcion_doc_encabezado="{{ $pedido->descripcion }}" data-lbl_total_factura="{{ '$ ' . number_format($pedido->valor_total,0,',','.') }}" data-nombre_vendedor="{{ $pedido->vendedor->tercero->descripcion }}" data-lineas_registros="{{ json_encode($pedido->lineas_registros_to_show()) }}" data-core_tipo_transaccion_id="{{ $pedido->core_tipo_transaccion_id }}" data-core_tipo_doc_app_id="{{ $pedido->core_tipo_doc_app_id }}"><i class="fa fa-btn fa-print"></i> Imprimir en cocina </a>
                        &nbsp;&nbsp;&nbsp;
                    @endif                
