{{ Form::open(['url'=>'vtas_doc_registro_guardar?id='.$id.'&id_modelo='.$id_modelo.'&id_transaccion='.$id_transaccion,'id'=>'form_edit']) }}
  ...
  @if($linea_factura->item->tipo == 'producto')
    <div class="row" style="padding:5px;">
  ...
  <input type="hidden" name="producto_id" id="producto_id" value="{{ $linea_factura->item->id}}">
  ...
  <input type="hidden" name="tipo" id="tipo" value="{{ $linea_factura->item->tipo }}">
