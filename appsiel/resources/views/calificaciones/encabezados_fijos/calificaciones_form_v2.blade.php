	...
	$(document).ready(function() {
		...
		*/
		...
		function calcular_definitiva_una_fila_promedio_poderado( numero_fila )
		{
			...
			const grupo_pesos = ['C1[]', 'C2[]', 'C3[]', 'C4[]'];

			// Por cada caja de texto de la fila
			var total_def = 0;
			var peso_columna = 0;
			var suma_calificaciones_grupo = parseFloat(0);
			var num_calificaciones_grupo = parseInt(0);
			$('.valores_' + numero_fila).each(function() {
				if ( grupo_pesos.includes($(this).attr('name')) ) {
					if ( this.value != 0 && this.value != '') {
						suma_calificaciones_grupo = suma_calificaciones_grupo + parseFloat(this.value);
						num_calificaciones_grupo++;	
					}
				}else{

					// Suma el peso (30%) del grupo
					peso_columna = 30;
					if (num_calificaciones_grupo != 0) {
						total_def += (suma_calificaciones_grupo / num_calificaciones_grupo) * (peso_columna / 100);
						num_calificaciones_grupo = 0; // Para que no vuelva a sumar
					}

					peso_columna = get_valor_peso_columna( $(this) );
		...
		function get_obj_boton_columna( obj_input_text )
		{
			...
			return celda_encabezado_columna.find('span');
		}
