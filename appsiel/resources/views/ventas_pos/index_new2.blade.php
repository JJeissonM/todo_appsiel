...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		$(".btn_acumular").click(function(event){
			...
			if ( !continuar )
				{
					...
					cantidad_facturas = arr_ids_facturas.length;
			...
			/*
			validar_existencias_new().then( acumular_new ).then(function() {
			...
			*/
			...
			//  
			function makeRequest(factura_id,contador){
		...
		function validar_existencias_new()
		{
			$('#contenido_modal').html( '<h1 style="text-align:center;"> <small>Por favor espere</small> <br> Validando Existencias... </h1>' );
		...
		function acumular_new()
		{
			...
			var allAJAX = arr_ids_facturas.map(factura_id => {
				...
				$.get("{{url('pos_acumular_una_factura')}}" + "/" + factura_id).then(function(){
					restantes--;
						document.getElementById('contador_facturas').innerHTML = restantes;
						console.log(restantes);
				});
			...
			/*var allAJAX = arr_ids_facturas.map(factura_id => {
				...
				return $.get( "{{url('pos_acumular_una_factura')}}" + "/" + factura_id ).then(function( data ) {
					restantes--;
					document.getElementById('contador_facturas').innerHTML = restantes;
					console.log(restantes);
