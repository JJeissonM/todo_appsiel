    ...
    <table class="table table-striped table-bordered">
        ...
        <tbody>
            @foreach ($pedidos as $pedido)
                <tr>
                    <td> {{ $pedido->fecha }} </td>
                    <td> {{ $pedido->tipo_documento_app->prefijo }} {{ $pedido->consecutivo }} </td>
                    <td> {{ $pedido->tercero->descripcion }} </td>
                    <td>
                        <a class="btn btn-default btn-xs btn-detail" href="{{ url( 'pos_factura_crear_desde_pedido/' . $pedido->id . '?id=20&id_modelo=230&id_transaccion=47&pdv_id=' . $pdv_id ) }}" title="Facturar" ><i class="fa fa-file"></i>&nbsp;</a>
                        &nbsp;&nbsp;&nbsp;
