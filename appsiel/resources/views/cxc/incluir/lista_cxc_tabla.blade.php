    ...
    <tbody>
        ...
        for($i=0;$i<count($movimiento_cxc);$i++){ 
            ...
            <tr id="{{ $id }}">
                ...
                <td class="col_valor_cartera text-right"> {{ number_format($movimiento_cxc[$i]['campo4'], 0, ',', '.') }} </td>
                <td class="col_saldo_pendiente text-right" > {{ number_format($movimiento_cxc[$i]['campo6'], 0, ',', '.') }} </td>
                <td> 
