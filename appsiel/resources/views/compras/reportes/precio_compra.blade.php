<h3 style="width: 100%; text-align: center;"> Estadísticas de precios de compras (Sin IVA) </h3>
<hr>
    ...
    <tbody>
        ...
        @foreach( $movimiento as $linea)
            ...
            <tr class="fila-{{$j}}">
                ...
                @php $precio_promedio = 0; if( $linea->cantidad != 0 ){ $precio_promedio = $linea->base_impuesto / $linea->cantidad; } @endphp
                <td> ${{ number_format( $precio_promedio, 2, ',', '.') }} </td>
                <td> ${{ number_format( $linea->base_impuesto, 2, ',', '.') }} </td>
            </tr>
         ...
         <?php
            ...
            $total_2_producto += $linea->base_impuesto;
