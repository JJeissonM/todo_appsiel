...
<div class="card">
    <div class="card-body d-flex justify-content-between flex-wrap">
        <div id="wrapper">
            <h4 class="column-title" style="padding: 10px;">Menú Prices</h4>
            @if($Price != null)
            <div class="descripcion" style="text-align: center; margin-top: 20px;">
                <h5 class="titulo">{{$Price->titulo}}</h5>
                <a href="{{url('prices/destroy').'/'.$Price->id.$variables_url}}" class="btn btn-lg" title="Eliminar Seccion"><i class="fa fa-window-close"></i> ELIMINAR SECCIÓN</a>
            </div>
            <div class="col-md-12 add d-flex">
                <div class="col-md-6">
                    <a href="{{url('prices/create').'/'.$widget.$variables_url}}" class="btn btn-primary waves-effect btn-block btn-sm" style="color: white; font-weight: bold;"> Agregar Plan </a>
                </div>
            ...
            <div class="col-md-12">
                <div class="contenido">
                    <div class="media service-box wow fadeInRight animated" style="visibility: visible; animation-name: fadeInRight;">
                        <div class="pull-left">
                            <img src="{{asset($item->imagen_cabecera)}}">
                        </div>
                    </div>
                    <div class="descripcion">
                        <h5 class="titulo">{{$item->precio}}</h5>
                    </div>
                    <a href="{{url('prices/edit').'/'.$item->id.$variables_url}}" class="btn" title="Editar Tarjeta"><i class="fa fa-edit"></i></a>
                    <a href="{{url('prices/destroy/item').'/'.$item->id.$variables_url}}" class="btn" title="Eliminar Tarjeta"><i class="fa fa-eraser"></i></a>
                </div>
            </div>
            @endforeach
            @endif
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div class="col-md-12">
                    {!! Form::open(['route'=>'prices.store','method'=>'POST','class'=>'form-horizontal','files'=>'true'])!!}
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div class="col-md-12">
                    @if($Price != null)
                    {!! Form::model($Price,['route'=>['prices.update',$Price],'method'=>'PUT','class'=>'form-horizontal','files'=>'true'])!!}
                    <input type="hidden" name="widget_id" value="{{$widget}}">
                    <input type="hidden" name="variables_url" value="{{$variables_url}}">
                    ...
                    <div class="form-group" id="fondo_container2">
                        ...
                        <a target="_blank" href="{{asset($Price->fondo)}}">Ver Actual</a><br>
                        <b>Repetir: {{$Price->repetir}}</b><br>
                        <b>Orientación Imagen: {{$Price->direccion}}</b>
                        @else
                        <label>Color de Fondo</label>
                        <div class="col-md-12" style="background-color: {{$Price->fondo}}; width: 100%; height: 20px;"></div>
                        @endif
                    </div>
                    <div class="form-group">
