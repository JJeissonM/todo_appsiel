...
<ul class="nav nav-tabs">
	...
	@foreach($ids_modelos_relacionados AS $key => $value)
		...
		<li class="{{$class_active}}">
			<a data-toggle="tab" href="{{$href}}" title="{{ $modelo_entidad->descripcion }}">
				<?php
					$descripcion_model =  $modelo_entidad->descripcion;
					if(strlen($modelo_entidad->descripcion) > 12){
						$descripcion_model = substr($modelo_entidad->descripcion,0,12) . '...';
					}
				?>
				<div style="border-left: 1px #ddd solid; padding-left:5px; width:100%;text-align:center;">{{ $descripcion_model }} </div>
			</a>
		</li>
		<?php $cont++; ?>
	@endforeach
