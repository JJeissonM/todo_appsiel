...
dd($json_doc_electronico_enviado);
   ...
   public function get_lineas_registros()
   {
      ...
      foreach ($lineas_registros as $linea)
      {
         ...
         $string_items .= '{"sku": "' . $linea->item->id . '","description": "' . $linea->item->descripcion . '","quantity": ' . abs( number_format( $linea->cantidad, $this->cantidadDecimales, '.', '') ) . ',"price": ' . abs( number_format($linea->base_impuesto, $this->cantidadDecimales, '.', '') ) . ',"discount_rate": ' . $linea->tasa_descuento . ',"taxes": [  {    "tax_rate": ' . $linea->tasa_impuesto . ',"tax_category": "IVA"}]}';
