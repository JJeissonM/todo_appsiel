    ...
    public function convert_to_electronic_invoice( $document_header_id )
    {
        ...
        return DB::transaction(function () use ($document_header_id, $modificado_por, $fe_document_type_id_default, $fe_transaction_type_id_default) {
            ...
            if ((int)$original_document_header->core_tipo_transaccion_id === $fe_transaction_type_id_default &&
                ...
                $existing_header = VtasDocEncabezado::where('core_empresa_id', $original_document_header->core_empresa_id)
                    ->where('core_tipo_transaccion_id', $fe_transaction_type_id_default)
                    ->where('core_tipo_doc_app_id', $fe_document_type_id_default)
                    ->where('consecutivo', $original_document_header->consecutivo)
                    ->orderBy('id', 'DESC')
                    ->first();

                return (object)[
                    'status' => 'flash_message',
                    'message' => 'El documento ' . $original_document_label . ' ya estaba convertido en Factura electrÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³nica.',
                    'new_document_header_id' => is_null($existing_header) ? 0 : $existing_header->id,
                    'was_already_converted' => 1
                ];
            }
            ...
            $cantidad = CxcAbono::where($array_wheres)->count();
            if ($cantidad != 0) {
                return (object)[
                    'status' => 'mensaje_error',
                    'message' => 'Factura ' . $original_document_label . ' NO puede ser convertida. Se le han hecho Recaudos de CXC (TesorerÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â­a).'
                ];
            }
            ...
            $cxc_movim = CxcMovimiento::where($array_wheres)->first();
            if (!is_null($cxc_movim)) {
