...
class VtasCotizacion extends Model
{
    ...
    protected $fillable = [ 'core_empresa_id', 'core_tipo_transaccion_id', 'core_tipo_doc_app_id', 'consecutivo', 'fecha', 'core_tercero_id', 'turno_operativo_id', 'descripcion', 'estado', 'creado_por', 'modificado_por', 'remision_doc_encabezado_id', 'ventas_doc_relacionado_id', 'cliente_id', 'contacto_cliente_id', 'vendedor_id', 'forma_pago', 'fecha_entrega', 'plazo_entrega_id', 'fecha_vencimiento', 'orden_compras', 'valor_total'];
    ...
    public static function consultar_registros($nro_registros, $search)
    {
        ...
        $query = VtasCotizacion::leftJoin('core_tipos_docs_apps', 'core_tipos_docs_apps.id', '=', 'vtas_doc_encabezados.core_tipo_doc_app_id')
    ...
    public static function sqlString($search)
    {
        ...
        $query = $query->where(function ($subquery) use ($search) {
            $subquery->where("vtas_doc_encabezados.fecha", "LIKE", "%$search%")
    ...
    public function almacenar_plazo_entrega( $modelo_padre_id, $registro_modelo_padre_id, $plazo_entrega )
    {
        $valor = implode(",", $plazo_entrega);
        
        ModeloEavValor::create( [ 
                                    "modelo_padre_id" => $modelo_padre_id,
                                    "registro_modelo_padre_id" => $registro_modelo_padre_id,
                                    "modelo_entidad_id" => $valor[0],
                                    "registro_modelo_entidad_id" => $valor[1]
                            ] );
    }
}
