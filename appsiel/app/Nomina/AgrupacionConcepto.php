...
class AgrupacionConcepto extends Model
{
    ...
    {
        $registros = AgrupacionConcepto::select(
            'nom_agrupaciones_conceptos.descripcion AS campo1',
            'nom_agrupaciones_conceptos.nombre_corto AS campo2',
            'nom_agrupaciones_conceptos.estado AS campo3',
            'nom_agrupaciones_conceptos.id AS campo4'
        )
            ->where("nom_agrupaciones_conceptos.descripcion", "LIKE", "%$search%")
            ->orWhere("nom_agrupaciones_conceptos.nombre_corto", "LIKE", "%$search%")
            ->orWhere("nom_agrupaciones_conceptos.estado", "LIKE", "%$search%")
    ...
    {
        return "LISTADO DE AGRUPACIÓN DE CONCEPTOS";
    }
