...
class SemanasCalendario extends Model
{
    ...
    {
        $string = SemanasCalendario::select('sga_semanas_calendario.numero AS NÚMERO', 'sga_semanas_calendario.descripcion AS DESCRIPCIÓN', 'sga_semanas_calendario.fecha_inicio AS FECHA_INICIAL', 'sga_semanas_calendario.fecha_fin AS FECHA_FINAL', 'sga_semanas_calendario.estado AS ESTADO')
            ->where("sga_semanas_calendario.numero", "LIKE", "%$search%")
            ->orWhere("sga_semanas_calendario.descripcion", "LIKE", "%$search%")
            ->orWhere("sga_semanas_calendario.fecha_inicio", "LIKE", "%$search%")
            ->orWhere("sga_semanas_calendario.fecha_fin", "LIKE", "%$search%")
