...
class CxCController extends TransaccionController
{
  ...
  public function store__no(Request $request,$registro_encabezado_doc)
  {
      /*
        ...
        ** Ahora por cada línea de servicio/concepto ingresada se debe llenar la tabla cxc_doc_registros y se va generando la contabilidad
        ** 
      */
      ...
      // El consecutivo que se envía en el request tiene valor 0, se debe actualizar
      $this->datos = array_merge( $request->all(), [ 'consecutivo' => $registro_encabezado_doc->consecutivo ] );

      // 1ro. se guardan los registros asociados al encabezado del documento (Cada una de las líneas ingresadas)
      // Se recorre la tabla enviada en el request, descartando las DOS últimas filas
      $valor_cartera = 0;
      $cant = count($tabla_registros_documento)-2;
      for ($i=0; $i < $cant; $i++) 
      {
          // Se obtienen las IDs de los campos que se van a almacenar.

          // Campo servicio
          $el_servicio = CxcServicio::find( $tabla_registros_documento[$i]->servicio_id );

          // Campo tercero
          $vec_tercero_id = explode("a3p0", $tabla_registros_documento[$i]->tercero_id );
          $tercero_id = $vec_tercero_id[0];
          $codigo_referencia_tercero = $vec_tercero_id[1];            

          // Campo valor
          $valor = $tabla_registros_documento[$i]->valor;

          // Descripcion
          if ( $request->descripcion != '') {
            $descripcion = $el_servicio->descripcion.' ('.$request->descripcion.')';
          }else{
            $descripcion = $el_servicio->descripcion;
          } 

          // $tipo_transaccion->modelo_registros_documentos equivale a CxcDocRegistro en este caso
          app($tipo_transaccion->modelo_registros_documentos)->create($this->datos +
          ...
          // MOVIMIENTO DEBITO (CARTERA)              
          $cuenta = Tercero::find($request->core_tercero_id)->cuenta_cartera;
          $valor_debito = $valor;
          $valor_credito = 0;
          
          $this->contabilizar_registro($cuenta->id, $descripcion, $valor_debito, $valor_credito);

          // MOVIMIENTO CREDITO (Contrapartida del motivo: INGRESOS, cuentas por paga, etc.)
          // Se cambia al tercero, si se seleccionó alguno al ingresar el servicio
          if ( $tercero_id != 0) {
            $this->datos = array_merge( $request->all(), [ 'consecutivo' => $registro_encabezado_doc->consecutivo, 'estado' => 'Pendiente', 'core_tercero_id' => $tercero_id, 'codigo_referencia_tercero' => $codigo_referencia_tercero ] );
          }

          $valor_debito = 0;
          $valor_credito = $valor;

          $this->contabilizar_registro($el_servicio->contab_cuenta_id, $descripcion, $valor_debito, $valor_credito);

      } // Fin Para cada línea de la tabla de registros

      // Se actualiza el encabezado del documento con el valor total
      $registro_encabezado_doc->valor_total = $valor_cartera;
      $registro_encabezado_doc->save();

      // Se almacena el movimiento de cartera 
      $cxc_movimiento = app($tipo_transaccion->modelo_movimientos)->create( $this->datos +  
                      [ 'documento_cartera_id' => $registro_encabezado_doc->id] + 
                      [ 'valor_cartera' => $valor_cartera] + 
                      [ 'saldo_pendiente' => $valor_cartera] +  
                      [ 'estado' => 'Pendiente'] +
                              [ 'detalle_operacion' => $descripcion]);

      // se llama la vista de CxCController@show
      return redirect( 'cxc/'.$registro_encabezado_doc->id.'?id='.$request->url_id.'&id_modelo='.$request->url_id_modelo );
  }
