...
class PlanillaIntegradaController extends Controller
{
    ...
    {
        return '<h4>Configuraciones de sucursal de autoliquidación de aportes pendientes.</h4><hr>';
    	$nom_doc_encabezado_id = $request->nom_doc_encabezado_id;
    	$encabezado_documento = NomDocEncabezado::find( $nom_doc_encabezado_id );

    	$archivo = new ArchivoPlanoPlanillaIntegrada( $encabezado_documento, file( $request->archivo_plano ) );
