...
class OdontologiaController extends ModeloController
{
    ...
    public function imprimir_historia_clinica( $consulta_id )
    {
        ...
        foreach ( $ids_modelos_relacionados as $key => $modelo_id )
        {
            if ( $modelo_id == 9999 )
            {
                $vistas_secciones .= '<br><br><br>' . View::make( 'consultorio_medico.odontologia.espacio_firmas', compact( 'consulta' ) )->render();
                continue;
            }
        ...
        $view =  View::make('consultorio_medico.odontologia.historia_clinica', compact( 'consulta', 'datos_historia_clinica', 'vistas_secciones', 'empresa'))->render();
