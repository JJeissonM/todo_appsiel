...
class NotaCreditoDirecta extends ComprasDocEncabezado
{
    ...
    {
        $core_tipo_transaccion_id = 40; // Nota crédito directa
        return NotaCreditoDirecta::leftJoin('core_tipos_docs_apps', 'core_tipos_docs_apps.id', '=', 'compras_doc_encabezados.core_tipo_doc_app_id')
    ...
    {
        $core_tipo_transaccion_id = 40; // Nota crédito directa
        $string = NotaCreditoDirecta::leftJoin('core_tipos_docs_apps', 'core_tipos_docs_apps.id', '=', 'compras_doc_encabezados.core_tipo_doc_app_id')
            ->leftJoin('core_terceros', 'core_terceros.id', '=', 'compras_doc_encabezados.core_tercero_id')
            ->where('compras_doc_encabezados.core_empresa_id', Auth::user()->empresa_id)
            ->where('compras_doc_encabezados.core_tipo_transaccion_id', $core_tipo_transaccion_id)
            ->select(
    ...
    {
        return "LISTADO DE NOTAS DE CREDITO DIRECTA";
    }
}
