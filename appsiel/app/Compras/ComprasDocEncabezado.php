...
class ComprasDocEncabezado extends Model
{
    ...
    protected $fillable = [
        ...
        'fecha_recepcion',
        'fecha_vencimiento',
        'doc_proveedor_prefijo',
        'doc_proveedor_consecutivo',
        'descripcion',
        'creado_por',
        'modificado_por',
        'estado',
    ...
    public static function consultar_registros($nro_registros, $search)
    {
        ...
        $collection = ComprasDocEncabezado::leftJoin('core_tipos_docs_apps', 'core_tipos_docs_apps.id', '=', 'compras_doc_encabezados.core_tipo_doc_app_id')
            ...
            ->leftJoin(DB::raw('(SELECT compras_doc_encabezado_id, SUM(CASE WHEN inv_producto_id IS NULL OR inv_producto_id = 0 THEN 1 ELSE 0 END) AS cant_sin_mapear FROM compras_doc_registros GROUP BY compras_doc_encabezado_id) AS registros_mapeo'), 'registros_mapeo.compras_doc_encabezado_id', '=', 'compras_doc_encabezados.id')
        ...
        CASE WHEN compras_doc_encabezados.sincronizado_bot = 1
            THEN " (Sinc. DIAN"
            ELSE ""
        END,
        CASE WHEN compras_doc_encabezados.sincronizado_bot = 1 AND registros_mapeo.cant_sin_mapear > 0 
            THEN CONCAT(" - Faltan ", CAST(registros_mapeo.cant_sin_mapear AS CHAR), " por mapear)") 
            WHEN compras_doc_encabezados.sincronizado_bot = 1 AND COALESCE(registros_mapeo.cant_sin_mapear, 0) = 0
            THEN " - Mapeado)"
            ELSE ""
        END
    ) AS campo8'),
    ...
    public static function get_registro_impresion($id)
    {
        ...
        return ComprasDocEncabezado::where('compras_doc_encabezados.id', $id)
            ...
            ->select(
                ...
                'compras_doc_encabezados.estado',
                'compras_doc_encabezados.sincronizado_bot',
