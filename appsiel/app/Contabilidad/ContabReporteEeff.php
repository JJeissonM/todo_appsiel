...
class ContabReporteEeff extends Model
{
    ...
    public static function consultar_registros($nro_registros, $search)
    {
        $registros = ContabReporteEeff::where('contab_reportes_eeff.core_empresa_id', Auth::user()->empresa_id)
            ->select(
                'contab_reportes_eeff.descripcion AS campo1',
                'contab_reportes_eeff.factor_expresion_valores AS campo2',
                'contab_reportes_eeff.id AS campo3'
            )
            ->orWhere("contab_reportes_eeff.descripcion", "LIKE", "%$search%")
            ->orWhere("contab_reportes_eeff.factor_expresion_valores", "LIKE", "%$search%")
    ...
    {
        $string = ContabReporteEeff::where('contab_reportes_eeff.core_empresa_id', Auth::user()->empresa_id)
            ->select(
                'contab_reportes_eeff.descripcion AS DESCRIPCIÓN',
                'contab_reportes_eeff.factor_expresion_valores AS EXPRESIÓN_DE_VALORES'
            )
            ->orWhere("contab_reportes_eeff.descripcion", "LIKE", "%$search%")
            ->orWhere("contab_reportes_eeff.factor_expresion_valores", "LIKE", "%$search%")
            ->orderBy('contab_reportes_eeff.created_at', 'DESC')
    ...
    {
        return "LISTADO DE REPORTES EEFF";
    }
