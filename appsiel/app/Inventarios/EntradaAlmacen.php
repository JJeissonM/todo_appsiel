...
class EntradaAlmacen extends InvDocEncabezado
{
    ...
    {
        $core_tipo_transaccion_id = 1; // Entrada de almacén

        $collection = EntradaAlmacen::leftJoin('core_tipos_docs_apps', 'core_tipos_docs_apps.id', '=', 'inv_doc_encabezados.core_tipo_doc_app_id')
    ...
    public static function sqlString($search)
    {
        ...
        return EntradaAlmacen::leftJoin('core_tipos_docs_apps', 'core_tipos_docs_apps.id', '=', 'inv_doc_encabezados.core_tipo_doc_app_id')
            ->leftJoin('core_terceros', 'core_terceros.id', '=', 'inv_doc_encabezados.core_tercero_id')
            ->leftJoin('inv_bodegas', 'inv_bodegas.id', '=', 'inv_doc_encabezados.inv_bodega_id')
            ->where('inv_doc_encabezados.core_empresa_id', Auth::user()->empresa_id)
            ->where('inv_doc_encabezados.core_tipo_transaccion_id', $core_tipo_transaccion_id)
    ...
    {
        return "LISTADO DE ENTRADAS DE ALMACEN";
    }

    public function crear_nueva( $modelo_id, $datos )
    {
        $encabezado_documento = $this->crear_encabezado( $modelo_id, $datos );

        $lineas_registros = json_decode( $datos['lineas_registros'] );
        $datos['consecutivo'] = $encabezado_documento->consecutivo;
