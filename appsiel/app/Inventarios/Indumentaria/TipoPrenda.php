...
{
    protected $table = 'inv_indum_tipos_prendas';
    ...
    {
        $collection =  TipoPrenda::select(
                'inv_indum_tipos_prendas.codigo AS campo1',
                'inv_indum_tipos_prendas.descripcion AS campo2',
                'inv_indum_tipos_prendas.estado AS campo3',
                'inv_indum_tipos_prendas.id AS campo4'
            )
            ->orderBy('inv_indum_tipos_prendas.created_at', 'DESC')
            ->get();
    ...
    {
        $string = TipoPrenda::select(
                'inv_indum_tipos_prendas.id AS CÓDIGO',
                'inv_indum_tipos_prendas.descripcion AS DESCRIPCIÓN',
                'inv_indum_tipos_prendas.estado AS ESTADO'
            )
            ->where("inv_indum_tipos_prendas.id", "LIKE", "%$search%")
            ->orWhere("inv_indum_tipos_prendas.descripcion", "LIKE", "%$search%")
            ->orWhere("inv_indum_tipos_prendas.codigo", "LIKE", "%$search%")
            ->orWhere("inv_indum_tipos_prendas.estado", "LIKE", "%$search%")
            ->orderBy('inv_indum_tipos_prendas.created_at', 'DESC')
            ->toSql();
    ...
    {
        $opciones = TipoPrenda::where('estado','Activo')
                            ->get();
        $vec['']='';
