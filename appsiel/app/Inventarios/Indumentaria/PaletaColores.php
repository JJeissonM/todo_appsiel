...
class PaletaColores extends Model
{
    protected $table = 'inv_indum_paletas_colores';
    ...
    {
        $collection =  PaletaColores::select(
                'inv_indum_paletas_colores.codigo AS campo1',
                'inv_indum_paletas_colores.descripcion AS campo2',
                'inv_indum_paletas_colores.estado AS campo3',
                'inv_indum_paletas_colores.id AS campo4'
            )
            ->orderBy('inv_indum_paletas_colores.created_at', 'DESC')
            ->get();
    ...
    {
        $string = PaletaColores::select(
                'inv_indum_paletas_colores.id AS CÓDIGO',
                'inv_indum_paletas_colores.descripcion AS DESCRIPCIÓN',
                'inv_indum_paletas_colores.estado AS ESTADO'
            )
            ->where("inv_indum_paletas_colores.id", "LIKE", "%$search%")
            ->orWhere("inv_indum_paletas_colores.descripcion", "LIKE", "%$search%")
            ->orWhere("inv_indum_paletas_colores.codigo", "LIKE", "%$search%")
            ->orWhere("inv_indum_paletas_colores.estado", "LIKE", "%$search%")
            ->orderBy('inv_indum_paletas_colores.created_at', 'DESC')
            ->toSql();
