...
class InvFisico extends InvDocEncabezado
{
    ...
    {
        $query = self::queryConsultarRegistros();

        if ( $search != '' )
        {
            $query->where(function ($query) use ($search) {
                $query->where("inv_doc_encabezados.fecha", "LIKE", "%$search%")
    ...
    {
        $string = self::queryConsultarRegistros([
    ...
    private static function queryConsultarRegistros($select = null)
    {
        $core_tipo_transaccion_id = 27; // Inventario Físico
        ...
        $query = InvFisico::leftJoin('core_tipos_docs_apps', 'core_tipos_docs_apps.id', '=', 'inv_doc_encabezados.core_tipo_doc_app_id')
        ...
        return self::aplicarFiltroCreadoPor($query, 'inv_doc_encabezados.creado_por');
    }
